...
(function registerPlayButtonForFirstSlide() {
  ...
  `;
    ...
    // Visible messages for the current image (only TypeIt-configured entries)
    let visibleMessages = [];
    // index within visibleMessages that is currently shown/shown-last (-1 = nothing shown yet)
    let visibleIndex = -1;
    ...
    // Build visibleMessages (only entries having TypeIt options)
    ...
    // initialize visibleMessages for the starting image
    visibleMessages = buildVisibleMessagesFor(currentSrcIndex);
    visibleIndex = -1;
    ...
    // Helper: show message at a specific index (index into visibleMessages)
    function showVisibleMessageAtIndex(idx) {
        ...
        // Use startTypeItOnElement with entry.text and entry.options
        startTypeItOnElement(textEl, entry.text, entry.options).catch(() => { });
        banner.classList.add('visible');
        visibleIndex = idx;
    }

    //UNTUK VIRTUAL ASSITANT
    function showAssistantChoices(onChoose) {
        ...
        // image file names per your request:
        const femaleImg = 'cewek_normal.jpeg';
        const maleImg = 'cowok_normal.jpeg';
